...
module.exports = {
   ...
   */
  join: (req, res, next) => {
    const courseId = req.params.id; // get the course id and current user from the request
  ...
  filterUserCourses: (req, res, next) => {
    ...
    if (currentUser) {
      const mappedCourses = res.locals.courses.map(course => { // Maps all courses
        const userJoined = currentUser.courses.some(userCourse => { // array.some() returns "true" if course._id matches a user's already joined courseId
